# UriPack Showcase: uribrowserdocker
...
Źródło runtime: `uribrowser/uribrowserdocker/manifest.yaml` · repo `uribrowser`

```yaml markpact:pack
apiVersion: urisys.io/v1
kind: UriPack
metadata:
  id: uribrowserdocker-showcase
  version: 0.1.0
  language: python
description: browser:// capability pack showcase.
schemes:
- browser
capabilities:
- id: browser-status
    ...
    if driver in ('cdp', 'remote-cdp'):
        ...
        with sync_playwright() as p:
            ...
            page.evaluate(
                ...
                """.find(el => /start a post/i.test(el.textCon

# ... truncated; see repo handlers.py ...
```
...
flow:
  id: browser-smoke
  description: Use case — smoke browser:// routes from uribrowserdocker manifest.
defaults:
  approved: true
  dry_run: true
do:
...
PACK=markpact-contracts/packs/uribrowserdocker.showcase.markpact.md

# 1) unpack: manifest + flows + proto → .markpact/{pack_id}/
urisys markpact materialize "$PACK"

# 2) pojedyncze URI (scheme {scheme}://)
urisys --packs none --markpact "$PACK" call 'browser://default/query/status' --approve --dry-run

# 3) osadzony flow use-case
urisys markpact run-flow "$PACK#browser-smoke" --approve --dry-run
...
- Handler refs: ``python://…`` — wymaga ``pip install -e uribrowser`` lub ``TELLMESH_ROOT``.
- Mock: ``--dry-run`` / ``environment: mock``.
- Integracje: ``uses:`` + ``urisys markpact run-flow`` ładuje sibling packi.

## Schemat URI

Wszystkie trasy: ``browser://`` (6 capability).
```
