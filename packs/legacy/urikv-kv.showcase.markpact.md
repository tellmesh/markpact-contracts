...
kind: UriPack
metadata:
  id: urikv-kv-showcase
  version: 0.1.0
  language: python
description: kv:// capability pack showcase.
schemes:
- kv
capabilities:
- id: kv-health
...
flow:
  id: kv-smoke
  description: Use case — smoke kv:// routes from urikv-kv manifest.
defaults:
  approved: true
  dry_run: true
do:
...
PACK=markpact-contracts/packs/urikv-kv.showcase.markpact.md

# 1) unpack: manifest + flows + proto → .markpact/{pack_id}/
urisys markpact materialize "$PACK"

# 2) pojedyncze URI (scheme {scheme}://)
urisys --packs none --markpact "$PACK" call 'kv://runtime/query/health' --approve --dry-run

# 3) osadzony flow use-case
urisys markpact run-flow "$PACK#kv-smoke" --approve --dry-run
...
Wszystkie trasy: ``kv://`` (8 capability).
```
