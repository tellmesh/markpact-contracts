...
Źródło runtime: `urillm/urillm/manifest.yaml` · repo `urillm`
...
flow:
  id: llm-smoke
  description: Use case — smoke llm:// routes from urillm manifest.
defaults:
  approved: true
  dry_run: true
do:
...
PACK=markpact-contracts/packs/urillm.showcase.markpact.md

# 1) unpack: manifest + flows + proto → .markpact/{pack_id}/
urisys markpact materialize "$PACK"

# 2) pojedyncze URI (scheme {scheme}://)
urisys --packs none --markpact "$PACK" call 'llm://local/vision/query/analyze' --approve --dry-run

# 3) osadzony flow use-case
urisys markpact run-flow "$PACK#llm-smoke" --approve --dry-run
...
- Handler refs: ``python://…`` — wymaga ``pip install -e urillm`` lub ``TELLMESH_ROOT``.
- Mock: ``--dry-run`` / ``environment: mock``.
- Integracje: ``uses:`` + ``urisys markpact run-flow`` ładuje sibling packi.

## Schemat URI

Wszystkie trasy: ``llm://`` (5 capability).
```
