...
Źródło runtime: `urikv/urikv/manifest.yaml` · repo `urikv`

```yaml markpact:pack
apiVersion: urisys.io/v1
kind: UriPack
metadata:
  id: urikv-log-showcase
  version: 0.1.0
  language: python
description: log:// capability pack showcase.
schemes:
- log
capabilities:
- id: log-health
...
flow:
  id: log-smoke
  description: Use case — smoke log:// routes from urikv-log manifest.
defaults:
  approved: true
  dry_run: true
do:
...
PACK=markpact-contracts/packs/urikv-log.showcase.markpact.md

# 1) unpack: manifest + flows + proto → .markpact/{pack_id}/
urisys markpact materialize "$PACK"

# 2) pojedyncze URI (scheme {scheme}://)
urisys --packs none --markpact "$PACK" call 'log://runtime/query/health' --approve --dry-run

# 3) osadzony flow use-case
urisys markpact run-flow "$PACK#log-smoke" --approve --dry-run
...
- Handler refs: ``python://…`` — wymaga ``pip install -e urikv`` lub ``TELLMESH_ROOT``.
- Mock: ``--dry-run`` / ``environment: mock``.
- Integracje: ``uses:`` + ``urisys markpact run-flow`` ładuje sibling packi.

## Schemat URI

Wszystkie trasy: ``log://`` (5 capability).
```
